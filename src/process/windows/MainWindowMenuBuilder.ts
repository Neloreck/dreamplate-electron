...
interface IDarwinMenuItemConstructorOptions extends MenuItemConstructorOptions {
  selector?: string;
  submenu?: Array<IDarwinMenuItemConstructorOptions> | Menu;
}

export class MainWindowMenuBuilder {

  public browserWindow: BrowserWindow;

  public constructor(mainWindow: AbstractWindow) {
    this.browserWindow = mainWindow.browserWindow;
  }

  public buildMenu(): Menu {
    if (processConfig.IS_DEVELOPMENT || processConfig.IS_DEBUG) {
  ...
  private setupDevelopmentEnvironment(): void {
    this.browserWindow.webContents.on("context-menu", (event: Event, props: ContextMenuParams) => {
      const { x, y } = props;
  ...
  private buildDarwinTemplate(): Array<MenuItemConstructorOptions> {
    ...
    const subMenuView: IDarwinMenuItemConstructorOptions =
      processConfig.IS_DEVELOPMENT || processConfig.IS_DEBUG ? subMenuViewDev : subMenuViewProd;
